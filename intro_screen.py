...
class IntroScreen:
    def __init__(self, screen, clock):
        self.screen = screen
        self.clock = clock
        # Use a default system font instead of joystix.ttf
        self.font = pygame.font.SysFont('Arial', 74)

        # Load and scale background image
        self.intro_bg = pygame.image.load('resources/textures/0.jpg').convert()
        ...
        self.blink_interval = 1000  # Blink interval in milliseconds (1 second)
    ...
    def show_intro(self):
        ...
        while intro:
            ...
            title_text = self.font.render('BLOODSTAINED DOOFESTIEN', True, (255, 255, 255))
            title_shadow = self.font.render('BLOODSTAINED DOOFESTIEN', True, (0, 0, 0))

            # Draw shadow first (slightly offset)
            self.screen.blit(title_shadow, (self.screen.get_width() // 2 - title_text.get_width() // 2 + 2,
                                            self.screen.get_height() // 3 + 2))
            # Draw the white text on top
            self.screen.blit(title_text, (self.screen.get_width() // 2 - title_text.get_width() // 2,
            ...
            if self.blink_state:
                ...
                # Draw shadow first (slightly offset)
                self.screen.blit(prompt_shadow, (self.screen.get_width() // 2 - prompt_text.get_width() // 2 + 2,
                                                 self.screen.get_height() // 2 + 2))
                # Draw the white text on top
                self.screen.blit(prompt_text, (self.screen.get_width() // 2 - prompt_text.get_width() // 2,
                                               self.screen.get_height() // 2))

            # Check for SPACE key press to exit the intro screen
